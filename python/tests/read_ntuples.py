...
if __name__ == "__main__":
    ...
    for branch in up_tree:
        ...
        if branch.typename != "std::string":  # Uproot cannot read strings?
            ...
            print(
                f"Uproot read {branch.name} into a {type(up_values)} and it has a mean of {np.nanmean(up_values):.2f}"
            )
    ...
    for branch in oxy_tree:
        ...
        if type(oxy_values) is np.ndarray:
            print(
                f"Oxyroot read {branch.name} into a {type(oxy_values)} and it has a mean of {np.nanmean(oxy_values):.2f}"
            )
        else:
            print(
                f"Oxyroot read {branch.name} into a {type(oxy_values)} and it has a length of {len(oxy_values)}"
            )
